...
renderText({
  paste("nrow(student_survey):", nrow(student_survey))
})

renderText({
  paste("students:", paste(student_survey$name, collapse = ", "))
})
...
```{r fig.width = 10}
ggplot(student_survey, aes(x = read_slides, fill = read_slides)) +
  geom_bar() +
  coord_flip() +
...
```{r fig.width = 10}
ggplot(student_survey, aes(x = hear_lecture, fill = hear_lecture)) +
  geom_bar() +
  coord_flip() +
  scale_fill_viridis_d() +
  guides(fill = "none") +
